        ...
        *,
        ...
        return await self._structured_completion(
            ...
            system_prompt=(
                ...
                "3. PRODUCT IDs: for products explicitly named in the latest message, match the "
                "complete exact brand/model/title only against catalogue.product_identities and "
                "return its supplied ID. Treat case, punctuation, and separator spacing as equivalent "
                "for identity matching, so S15 matches S 15. A shared brand or product family is not "
                "an exact match: "
                "for example, iPhone 14 must never return an iPhone 16 ID. Never substitute a closest "
                "model. Whenever referenced_product_ids is non-empty—including "
                "for an explicitly named BUY—reference_status must be RESOLVED. When no exact "
                "identity matches, keep the IDs empty and status NONE, preserve the requested model "
                "in search_query, and do not ask for category or budget. A first-turn comparison "
                "such as 'A vs B' must return both IDs, set reference_status RESOLVED, and must not "
                "ask for category or budget. For pronouns, ordinals, 'it', or 'those', resolve only "
                "against conversation.allowed_previous_products in its stable order. "
                "client_selected_product_id is valid "
