...
import type { AccountProfile, AgentChatRequest, AgentChatResponse, AgentConversationDetail, AgentConversationSummary, CatalogProduct, PurchaseProposal } from '../types'
...
export default function AgentWorkspace({ profile, sessionChecked, onSignIn, onAddToCart }: { profile: AccountProfile | null; sessionChecked: boolean; onSignIn: () => void; onAddToCart: (product: CatalogProduct) => void }) {
  ...
  function recommendationCards(response: AgentChatResponse) {
    return <>
      ...
      <div className="agent-recommendations">{response.recommendations.map((item) => <article className="agent-recommendation" key={item.product.id}><div>{response.exact_match && response.focus_product_id === item.product.id ? <span className="exact-badge">Exact catalogue match</span> : null}<h3>{item.product.title}</h3><p>{item.reasons.slice(0, 2).join(' · ')}</p><strong>{money(item.product.offer_price_paise)}</strong></div><small>{item.score}/100</small><div className="agent-recommendation-actions"><button type="button" onClick={() => submit(`Choose ${item.product.title}`, { selected_product_id: item.product.id })}>Buy with Agent</button><button type="button" onClick={() => onAddToCart(item.product)}>Add to normal cart</button></div></article>)}</div>
      {response.cross_sell ? <article className="agent-recommendation"><div><span className="exact-badge">Optional add-on</span><h3>{response.cross_sell.product.title}</h3><strong>{money(response.cross_sell.product.offer_price_paise)}</strong></div><div className="agent-recommendation-actions"><button type="button" onClick={() => onAddToCart(response.cross_sell!.product)}>Add separately to cart</button></div></article> : null}
  ...
  return <main className="agent-workspace">
    ...
    <div className="agent-layout">
      ...
      <aside className="agent-inspector">{proposal ? <AgentCheckout proposal={proposal} signedIn={profile !== null} onSignIn={onSignIn} onRunChange={() => setHistoryKey((value) => value + 1)}/> : <section className="agent-checkout-gate"><strong>No purchase selected</strong><p>Choose a recommendation to review its immutable quote and policy checks.</p></section>}{profile ? <AgentRunHistory refreshKey={historyKey}/> : null}{latest?.notice ? <p className="agent-empty-copy">{latest.notice}</p> : null}</aside>
    </div>
  </main>
}
