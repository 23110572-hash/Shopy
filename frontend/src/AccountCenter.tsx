    ...
    <main className="account-page">
      <section className="account-hero">
        <div>
          ...
          {sessionState === 'authenticated' ? <p>Manage your account.</p> : null}
        </div>
      ...
      <section className="account-workspace">
        ...
        <div className="account-content">
          ...
          {activeTab === 'overview' && profile ? <section className="account-panel overview-panel">
            <header><div><span>ACCOUNT OVERVIEW</span><h2>Your details</h2></div><b className={profile.is_active ? 'account-state active' : 'account-state'}>{profile.is_active ? 'Active' : 'Disabled'}</b></header>
          ...
          {activeTab === 'orders' ? <section className="account-panel history-panel"><header><div><span>ORDER HISTORY</span><h2>Your orders</h2><p>Open an order to see the product and provider details.</p></div><b>{orders?.items.length ?? 0} orders</b></header>{orders?.items.length ? <div className="history-list">{orders.items.map((order) => <article key={order.order_id}><button className="history-summary" type="button" aria-expanded={openOrderId === order.order_id} onClick={() => setOpenOrderId((current) => current === order.order_id ? null : order.order_id)}><span><strong>{order.product_title}</strong><small>{order.product_brand} · {order.product_model} · Qty {order.quantity}</small></span><strong>{formatPrice(order.amount_paise)}</strong><div className="history-meta"><span className={`status-badge status-${order.status.toLowerCase()}`}>{order.status}</span><small>{formatDate(order.created_at)}</small></div><i>{openOrderId === order.order_id ? '−' : '+'}</i></button>{openOrderId === order.order_id ? <dl className="history-detail"><div><dt>Product</dt><dd>{order.product_title}</dd></div><div><dt>Razorpay order</dt><dd>{order.provider_order_id}</dd></div><div><dt>Amount</dt><dd>{formatPrice(order.amount_paise)}</dd></div><div><dt>Updated</dt><dd>{formatDate(order.updated_at)}</dd></div></dl> : null}</article>)}</div> : <HistoryEmpty kind="order" title="No authoritative orders yet" reason={orders?.reason ?? 'Loading order history…'} />}</section> : null}
